...
public class SLinkedList<E> {
	...
	E get(int index) {
		if (index == 0) {
			return head.item;// head의 item을 반환한다.
		}
		return head.item;
	}
...
//        newNode.next = tail;
	...
	E remove() { // removeFirst()를 호출
		Node<E> removedNode = head; // 노드를 지우기 전에, head에 지워질 노드 E 타입을 백업한다.
		head = head.next; // null 값을 head로 할당시키면서 Head는 removeNide와의 연결이 끊긴다.
